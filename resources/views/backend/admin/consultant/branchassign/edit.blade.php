        ...
        <div class="card">
            <div class="d-flex justify-content-between bg-success-subtle px-4 pt-3">
                ...
                <div class="text-end pt-2">
                    <a href="{{ route('branch.manager.index') }}" class="btn btn-primary"><i class="fa-regular fa-eye"></i>
                        View List</a>
                </div>
            </div>
            <form method="" action="" enctype="multipart/form-data" class="row g-3 p-3 needs-validation"
                ...
                <div class="col-md-6 pb-3">
                    <label for="region_id" class="form-label">Branch Manager <span class="text-danger">*</span></label>
                    <select class="js-example-basic-single form-select" name="region_id" id="region_id">
                        <option selected disabled>Select Branch Manager</option>
                        @foreach ($countries as $key => $country)
                            <option value="{{ $country->id }}">{{ $country->name }}</option>
                        @endforeach
                    </select>
