...
@section('content')
    <main id="main" class="main">
        ...
        <hr>
        <div class="card">
            <form method="post" action="{{ route('degreemapping.update', $degreemapping->id) }}"
                    ...
                    <select id="campus_id" name="campus_id" class="form-select" required>
                        @foreach ($campuses as $campus)
                            <option value="{{ $campus->id }}"
                                {{ $campus->id == $degreemapping->campus_id ? 'selected' : '' }}>
                                {{ $campus->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="degree_id" name="degree_id" class="form-select" required>
                        @foreach ($degrees as $degree)
                            <option value="{{ $degree->id }}"
                                {{ $degree->id == $degreemapping->degree_id ? 'selected' : '' }}>
                                {{ $degree->name }}
                            </option>
                        @endforeach
                    </select>
