...
@section('content')
    <main id="main" class="main">
        ...
        <hr>
        <div class="card">
            <form method="post" action="{{ route('university.store') }}" enctype="multipart/form-data" class="row g-3 p-3">
                @csrf
                ...
                <div class="col-md-12 pb-3" id="rowWrapper">
                    ...
                    <div class="d-flex mb-3 rowItem">
                        <input type="text" class="form-control me-2" name="name[]" placeholder="University Name" value="{{ old('name') }}" required>
                        @error('name')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
